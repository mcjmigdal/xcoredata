...
overlap_mat_symbol <- rowsum(x = overlap_mat_promoters_for_gene_collapsing, 
			     group = promoters_ext500_for_gene_collapsing$SYMBOL)
overlap_mat_symbol[overlap_mat_symbol > 0] <- 1

overlap_mat_entrez <- rowsum(x = overlap_mat_promoters_for_entrez_collapsing, 
			     group = promoters_ext500_for_entrez_collapsing$ENTREZID)
